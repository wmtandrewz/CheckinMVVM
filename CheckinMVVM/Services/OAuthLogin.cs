    ...
    public class OAuthLogin
    {
            ...
            try
            {
                ...
                using (var client = new HttpClient())
                {
                    ...
                    else if (response.StatusCode == System.Net.HttpStatusCode.InternalServerError)
                    {
                        ...
                        {
                            string result = await content.ReadAsStringAsync();
                            return result.Contains("The request scope is not valid or is unsupported") ? "Gateway URL error" : "";
                        }
        ...
        public async Task<string> GetAccessToken(string payload)
        {
            try
            {
                using (var client = new HttpClient())
                {
                    client.BaseAddress = new Uri($"{Settings.GatewayUri}/sap/bc/sec/oauth2/token");
                    ...
                    HttpResponseMessage response = await client.PostAsync($"{Settings.GatewayUri}/sap/bc/sec/oauth2/token", httpContent);
                    if (response.IsSuccessStatusCode)
                    {
                        using (HttpContent content = response.Content)
                        {
                            try
                            {
                                ...
                                Constants.AccessToken = jObj["access_token"].ToString();
                                Constants.TokenExpiresIn = jObj["expires_in"].ToString();

                                ////Store Access Token and Expiration Time
                                //Settings.AccessToken = Constants._access_token;
                                //Settings.ExpiresTime = DateTime.Now.AddMinutes(55).ToString();

                                return $"SUCCESS {jObj["access_token"].ToString()}";
                            }
                            catch (Exception ex)
                            {
                                return ex.Message;
